<?php
/* @var $this GaleriaController */
/* @var $dataProvider CActiveDataProvider */

$this->pageTitle=Yii::app()->name . ' - Vinilos';

$this->breadcrumbs=array(
	'Elementos',
);

$this->menu=array(
	array('label'=>'Create Elemento', 'url'=>array('create')),
	array('label'=>'Manage Elemento', 'url'=>array('admin')),
);
?>

<?php $this->widget('zii.widgets.CListView', array(
	'dataProvider'=>$dataProvider,
	'itemView'=>'_view',
)); ?>

<ul class="portfolio">
    <li>
        <a href="portfolio-item.html">
            <img src="placeholders/360x225/1.jpg" alt="">
            <div class="overlay">
                <h3>Lorem ipsum dolor sit amet</h3>
                <p>Morbi eu nibh nibh, ut cursus leo</p>
            </div>
        </a>
    </li>
    <li>
        <a href="portfolio-item.html">
            <img src="placeholders/360x225/2.jpg" alt="">
            <div class="overlay">
                <h3>Morbi eu nibh nibh</h3>
                <p>Pellentesque vitae libero non</p>
            </div>
        </a>
    </li>
    <li>
        <a href="portfolio-item.html">
            <img src="placeholders/360x225/3.jpg" alt="">
            <div class="overlay">
                <h3>Integer ante mauris</h3>
                <p>Aenean hendrerit, tortor et ullamcorper</p>
            </div>
        </a>
    </li>
    <li>
        <a href="portfolio-item.html">
            <img src="placeholders/360x225/4.jpg" alt="">
            <div class="overlay">
                <h3>Aliquam feugiat scelerisque</h3>
                <p>Duis vulputate pulvinar congue</p>
            </div>
        </a>
    </li>
    <li>
        <a href="portfolio-item.html">
            <img src="placeholders/360x225/5.jpg" alt="">
            <div class="overlay">
                <h3>Integer ante mauris</h3>
                <p>Nam egestas tempor tortor</p>
            </div>
        </a>
    </li>
    <li>
        <a href="portfolio-item.html">
            <img src="placeholders/360x225/6.jpg" alt="">
            <div class="overlay">
                <h3>Phasellus vestibulum</h3>
                <p>Donec vel tortor dui</p>
            </div>
        </a>
    </li>
    <li>
        <a href="portfolio-item.html">
            <img src="placeholders/360x225/7.jpg" alt="">
            <div class="overlay">
                <h3>Nam eu orci nisl</h3>
                <p>Vivamus tincidunt, leo at congue</p>
            </div>
        </a>
    </li>
    <li>
        <a href="portfolio-item.html">
            <img src="placeholders/360x225/8.jpg" alt="">
            <div class="overlay">
                <h3>Pellentesque dignissim</h3>
                <p>Proin luctus, risus nec elementum</p>
            </div>
        </a>
    </li>
    <li>
        <a href="portfolio-item.html">
            <img src="placeholders/360x225/9.jpg" alt="">
            <div class="overlay">
                <h3>Mauris venenatis ligula</h3>
                <p>Lorem ipsum dolor sit amet</p>
            </div>
        </a>
    </li>
    <li>
        <a href="portfolio-item.html">
            <img src="placeholders/360x225/10.jpg" alt="">
            <div class="overlay">
                <h3>Nam eu orci nisl</h3>
                <p>Mauris venenatis ligula</p>
            </div>
        </a>
    </li>
    <li>
        <a href="portfolio-item.html">
            <img src="placeholders/360x225/11.jpg" alt="">
            <div class="overlay">
                <h3>Mauris venenatis ligula</h3>
                <p>Phasellus malesuada nulla quis</p>
            </div>
        </a>
    </li>
    <li>
        <a href="portfolio-item.html">
            <img src="placeholders/360x225/12.jpg" alt="">
            <div class="overlay">
                <h3>Suspendisse et nunc</h3>
                <p>Phasellus vestibulum accumsan</p>
            </div>
        </a>
    </li>
    <li>
        <a href="portfolio-item.html">
            <img src="placeholders/360x225/13.jpg" alt="">
            <div class="overlay">
                <h3>Integer ante mauris</h3>
                <p>Aenean hendrerit, tortor et ullamcorper</p>
            </div>
        </a>
    </li>
    <li>
        <a href="portfolio-item.html">
            <img src="placeholders/360x225/14.jpg" alt="">
            <div class="overlay">
                <h3>Aliquam feugiat scelerisque</h3>
                <p>Duis vulputate pulvinar congue</p>
            </div>
        </a>
    </li>
    <li>
        <a href="portfolio-item.html">
            <img src="placeholders/360x225/15.jpg" alt="">
            <div class="overlay">
                <h3>Integer ante mauris</h3>
                <p>Nam egestas tempor tortor</p>
            </div>
        </a>
    </li>
    <li>
        <a href="portfolio-item.html">
            <img src="placeholders/360x225/16.jpg" alt="">
            <div class="overlay">
                <h3>Phasellus vestibulum accumsan</h3>
                <p>Donec vel tortor dui</p>
            </div>
        </a>
    </li>
    <li>
        <a href="portfolio-item.html">
            <img src="placeholders/360x225/17.jpg" alt="">
            <div class="overlay">
                <h3>Nam eu orci nisl</h3>
                <p>Vivamus tincidunt, leo at congue</p>
            </div>
        </a>
    </li>
    <li>
        <a href="portfolio-item.html">
            <img src="placeholders/360x225/18.jpg" alt="">
            <div class="overlay">
                <h3>Pellentesque dignissim</h3>
                <p>Proin luctus, risus nec elementum</p>
            </div>
        </a>
    </li>
    <li>
        <a href="portfolio-item.html">
            <img src="placeholders/360x225/19.jpg" alt="">
            <div class="overlay">
                <h3>Mauris venenatis ligula</h3>
                <p>Lorem ipsum dolor sit amet</p>
            </div>
        </a>
    </li>
    <li>
        <a href="portfolio-item.html">
            <img src="placeholders/360x225/20.jpg" alt="">
            <div class="overlay">
                <h3>Nam eu orci nisl</h3>
                <p>Mauris venenatis ligula</p>
            </div>
        </a>
    </li>
</ul>

